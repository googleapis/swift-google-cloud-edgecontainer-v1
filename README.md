...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-edgecontainer-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudEdgeContainerV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudEdgeContainerV1 <target-name> --package swift-google-cloud-edgecontainer-v1
```
